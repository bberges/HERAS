...
#path <- 'C:/Workspace/stox/HERAS_plot/HERAS/'
path <- 'C:/git/HERAS/'
...
for(idxYear in 1:length(surveyYearMat)){
  ...
  for(idxDataDir in 1:length(dataDirs)){
    ...
    tempVal       <- strsplit(projectPath,'/')
    projectName   <- tempVal[[1]][length(tempVal[[1]])]
    ...
    #write.csv(spp_data_int,file.path(outPath,'mergedNASCS14.csv'),row.names = F)
    ...
    mlondist <- aggregate(cbind(Dist, SA,Dist*SA) ~ FID, resample.all.transects, sum)
    colnames(mSHORTdist) <- colnames(mlondist)
    ...
    tt <-  merge(resample.transects.ID,malldist,by = "FID",incomparables = F)
    malldist <-  tt[!duplicated(tt$FID),]
    
    ###### !!!! #####
    mdist <- aggregate(cbind(Dist, SA,Dist*SA) ~ FID, resample.all.transects, sum)# get the mean SA of 5 nmi distances
    ...
    #write.csv(transects.resampled ,file.path(outPath,'HERAS_FEB19.csv'),row.names = F)
    ...
    indBiotic<- indBiotic[  !is.na(as.numeric(indBiotic$age))&indBiotic$aphia==126417 | 
                            !is.na(as.numeric(indBiotic$age))&indBiotic$aphia==126425,]
    ...
    data_abu$Abundance <- as.numeric(data_abu$Abundance)
    data_abu$Abundance[is.na(data_abu$Abundance)] <- 0
    ...
    write.csv(nasc.maturity,file.path(outPath,paste0(projectName,'_table.csv')), row.names = F)
  }
}