...
ProjectName<-   "2018207_HERAS2018_NOR - v2.6 - length"
#ProjectName<-   "01 Post-cruise - HERAS_2018_HER" "MSHAS_2018_HER_MO"
...
only.transect <- spp_data_int[!is.na(spp_data_int$Transect), ]
...
for (i in unique(only.transect$TransectNo))
{
  ...
  d1 <- as.numeric(duplicated(mTransects$inid_one,fromLast=FALSE ))
  d2 <- as.numeric(duplicated(mTransects$inid_one,fromLast=TRUE ))
  
  mTransects$duplicates <- apply(as.data.frame(cbind(d1,d2)),1,max)
...
short.transects <- resample.transects[resample.transects$duplicates==1,]
...
mdistsum <- aggregate(cbind(Dist, SA,Dist*SA) ~ FID, malldist, sum)
mdist <- aggregate(cbind(Dist, SA,Dist*SA) ~ FID, malldist, mean)
mdist <- aggregate(cbind(Dist, SA,SA) ~ FID, resample.all.transects,mean )

mdist$mSA <- (mdist$SA) # mSa is the mean SA of 5 nmi distances
...
######
# End..
write.csv(nasc.maturity,"nasc.maturity.NORWAY_T2.csv", row.names = F)
#source("stox_plot.r")
#nasc.maturity$maturperc # This is the NASC splited as  Matur
#nasc.maturity$immaturperc # This is the NASC splited as  Immature
######
